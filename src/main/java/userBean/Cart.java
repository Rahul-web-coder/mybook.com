...
public class Cart {
	...
	public int checkDuplicateProductfromCart() throws SQLException {
			
			Connection conn = DBConnection.dbconnect();
			String sql = "select count(*) as cartlength from cart where userEmail=? and pName=?";
			java.sql.PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, userEmail);
			...
			int counter1=0;
			if(rs.next()) {
				counter1=rs.getInt("cartlength");
			}
			conn.close();
			return counter1;
	}
	
	public int updateDuplicateProductfromCart() throws SQLException {
		
		Connection conn = DBConnection.dbconnect();
		String sql = "update cart set pQty= PQty + 1 where userEmail=? and pName=?";
		java.sql.PreparedStatement ps = conn.prepareStatement(sql);
		ps.setString(1, userEmail);
		ps.setString(2, pName);
		int updateduplicateprod=ps.executeUpdate();
		conn.close();
